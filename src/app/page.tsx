...
export default function Home() {
  ...
  const handleSelectPharmacy = async (pharmacy: SearchSuggestion) => { const accessToken = process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN; const endpoint = `https://api.mapbox.com/search/searchbox/v1/retrieve/${pharmacy.mapbox_id}?access_token=${accessToken}&session_token=${sessionToken}`; try { const response = await fetch(endpoint); const data = await response.json(); const feature = data.features[0]; const street = feature.properties.address || ''; const city = feature.properties.context.place?.name || ''; const postcode = feature.properties.context.postcode?.name || ''; const state = feature.properties.context.region?.region_code?.replace('US-', '') || ''; const phone = feature.properties.phone || ''; const [longitude, latitude] = feature.geometry.coordinates; setSelectedPharmacy({ mapbox_id: pharmacy.mapbox_id, name: pharmacy.name, full_address: pharmacy.full_address, street_address: street, city: city, state: state, zip_code: postcode, latitude: latitude, longitude: longitude, phone_number: phone, }); setSearchTerm(''); setResults([]); } catch (error) { console.error("Error retrieving pharmacy details:", error); } };
  const handleZipCodeSubmit = async (e: FormEvent) => { e.preventDefault(); setIsLoadingLocation(true); setLocationError(""); const accessToken = process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN; const endpoint = `https://api.mapbox.com/geocoding/v5/mapbox.places/${zipCode}.json?access_token=${accessToken}&types=postcode`; try { const response = await fetch(endpoint); const data = await response.json(); if (data.features && data.features.length > 0) { const [lon, lat] = data.features[0].center; setLocationCoords([lat, lon]); } else { setLocationError("Could not find that ZIP code. Please try another."); } } catch { setLocationError("Failed to fetch location data."); } setIsLoadingLocation(false); };
  ...
  const handleFilterClick = (days: number | null) => { if (dateFilter === days) { setDateFilter(null); } else { setDateFilter(days); } };

  if (!locationCoords) { return ( <div className={styles.zipCodeContainer}><form onSubmit={handleZipCodeSubmit}><h2 >Enter Your ZIP Code</h2><p style={{ color: 'var(--font-color-dark)', fontStyle: 'italic' }}>Please enter a ZIP code to find pharmacies with bupe near you.</p><div className={styles.formGroup}><label htmlFor="zip-code">ZIP Code</label><input id="zip-code" type="text" inputMode="numeric" pattern="[0-9]{5}" maxLength={5} value={zipCode} onChange={(e) => setZipCode(e.target.value)} placeholder="e.g., 49506" required /></div><button type="submit" className={styles.submitButton} disabled={isLoadingLocation}>{isLoadingLocation ? 'Loading...' : 'Set Location'}</button>{locationError && <p className={styles.errorText}>{locationError}</p>}</form></div> ); }
  if (!mode) { return ( <div className={styles.choiceContainer}><h2>What would you like to do?</h2>{stats && stats.weeklyCount > 0 && (<p className={styles.statsTextOnDark}>Join {stats.weeklyCount} {stats.weeklyCount === 1 ? 'person' : 'people'} who have submitted a report this week!</p>)}<div className={styles.choiceButtons}><button className={styles.choiceButton} onClick={() => setMode('report')}>Update a Pharmacy&apos;s Bupe Status</button><button className={styles.choiceButton} onClick={() => setMode('find')}>Find a Bupe-Friendly Pharmacy</button></div></div> ); }

  return (
    <div className={styles.homeContainer}>
      <section className={styles.mapSection}><MapLoader center={locationCoords} pharmacies={aggregatedPharmacies} /></section>
      <section className={styles.formSection}>
        {mode === 'find' && ( <div className={styles.printableArea}><div className={styles.findHeader}><h2>Bupe-Friendly Pharmacies</h2><button onClick={() => window.print()} className={styles.printButton}>Print List</button></div><div className={styles.trendLegend}>
            ...
            </div><div className={styles.filterGroup}><button onClick={() => handleFilterClick(7)} className={`${styles.filterButton} ${dateFilter === 7 ? styles.active : ''}`}>Last 7 Days</button><button onClick={() => handleFilterClick(15)} className={`${styles.filterButton} ${dateFilter === 15 ? styles.active : ''}`}>Last 15 Days</button><button onClick={() => handleFilterClick(30)} className={`${styles.filterButton} ${dateFilter === 30 ? styles.active : ''}`}>Last 30 Days</button><button onClick={() => handleFilterClick(null)} className={`${styles.filterButton} ${styles.allTimeButton} ${dateFilter === null ? styles.active : ''}`}>All Time</button></div><div className={styles.pharmacyList}>{isLoadingPharmacies ? (<p>Loading pharmacies...</p>) : successfulPharmacies.length > 0 ? ( successfulPharmacies.map(pharmacy => ( <PharmacyListItem key={pharmacy.id} pharmacy={pharmacy} /> )) ) : (<p>No successful reports found for the selected time period. Try a wider date range or select &quot;All Time&quot;.</p>)}</div><div className={styles.printFooter}><p>For the latest updates, visit bupe.opioidpolicy.org</p></div></div> )}
        {mode === 'report' && (
          <>
            {submitStatus === 'success' ? (
              <div className={styles.successMessage}>
                <h3>🎉 Thank You! 🎉</h3>
                {finalStats && (<p className={styles.statsTextOnLight}>{finalStats.zipCodeCount === 1 ? ( "Congratulations on being the first submission in your area! This is an important step towards increasing access to bupe in your community." ) : ( <>You are part of {finalStats.totalCount} total contributions. {finalStats.zipCodeCount && finalStats.zipCodeCount > 0 ? ` In your area, ${finalStats.zipCodeCount} reports have been made!` : ''}</>)}</p>)}
              ...
              <form onSubmit={handleSubmit}>
                ...
                {!selectedPharmacy ? (
                  <>
                    <div className={styles.formGroup}><label htmlFor="pharmacy-search">Search for a Pharmacy</label><input type="text" id="pharmacy-search" placeholder="e.g., CVS on 28th St" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
                    <div className={styles.resultsList}>{results.map((result) => (<button key={result.mapbox_id} type="button" className={styles.resultItem} onClick={() => handleSelectPharmacy(result)}><strong>{result.name}</strong><small>{result.full_address}</small></button>))}</div>
                  </>
                ) : (
                  <>
                    <div className={styles.selectedPharmacy}><p><strong>{selectedPharmacy.name}</strong><br />{selectedPharmacy.full_address}</p><button type="button" onClick={() => setSelectedPharmacy(null)}>Change pharmacy</button></div>
                    <div className={styles.formGroup}>
                      <label>What was the outcome?</label>
                      <hr className={styles.formDivider} />
                      <div className={styles.radioGroup}>
                        <label><input type="radio" name="reportType" value="success" checked={reportType === 'success'} onChange={(e) => setReportType(e.target.value as 'success' | 'denial')} /> Success</label>
                        <label><input type="radio" name="reportType" value="denial" checked={reportType === 'denial'} onChange={(e) => setReportType(e.target.value as 'success' | 'denial')} /> Denial / Refusal</label>
                      </div>
                    </div>
                    <div className={styles.formGroup}>
                      <label>Formulation (optional, check all that apply)</label>
                      <hr className={styles.formDivider} />
                      <div className={styles.checkboxGroup}>{formulationOptions.map(option => (<label key={option} className={styles.checkboxLabel}><input type="checkbox" value={option} checked={formulations.includes(option)} onChange={() => handleFormulationChange(option)}/>{option}</label>))}</div>
                    </div>
                    <div className={styles.formGroup}>
                      <label>Common Observations (optional)</label>
                      <hr className={styles.formDivider} />
                      <div className={styles.checkboxGroup}>{standardizedNoteOptions.map(option => (<label key={option} className={styles.checkboxLabel}><input type="checkbox" value={option} checked={standardizedNotes.includes(option)} onChange={() => handleStandardizedNoteChange(option)} />{option}</label>))}</div>
                    </div>
                    <div className={styles.formGroup}>
                      <label htmlFor="notes">Other Notes (optional)</label>
                      <hr className={styles.formDivider} />
                      <textarea id="notes" placeholder="e.g., &apos;Out of business.&apos;" rows={4} value={notes} onChange={(e) => setNotes(e.target.value)}></textarea>
                    </div>
